...
<div class="space-y-4">
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-xl font-semibold flex items-center gap-2"><LayoutGrid size={20} /> Custom Views</h1>
      <p class="text-sm text-base-content/50">Saved views for collections</p>
    </div>
    <button class="btn btn-primary btn-sm gap-1" onclick={() => (showForm = true)}><Plus size={14} /> New view</button>
  </div>
  ...
  {:else}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
      {#if views.length === 0}
        <div class="col-span-full card bg-base-200"><div class="card-body items-center py-12 text-base-content/50 text-sm">No saved views yet.</div></div>
      {:else}
        {#each views as v (v.id)}
          ...
          <div class="card bg-base-200 border border-base-300">
            <div class="card-body p-4 gap-2">
              ...
              <div class="flex justify-end"><button class="btn btn-ghost btn-xs text-error" onclick={() => deleteView(v.id)}>Delete</button></div>
            </div>
          </div>
        {/each}
      {/if}
...
{#if showForm}
  <div class="modal modal-open">
    <div class="modal-box max-w-lg">
      <div class="flex items-center justify-between mb-4"><h3 class="font-semibold">New view</h3><button class="btn btn-ghost btn-xs" onclick={() => (showForm = false)}><X size={14} /></button></div>
      <div class="space-y-3">
        ...
        <div class="form-control"><label class="label py-0"><span class="label-text text-xs">View type</span></label>
        ...
        <div class="form-control"><label class="label py-0"><span class="label-text text-xs">Config (JSON)</span></label><textarea class="textarea textarea-sm font-mono text-xs" rows="7" bind:value={form.config}></textarea></div>
      </div>
