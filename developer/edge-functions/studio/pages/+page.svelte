...
<div class="flex h-[calc(100vh-12rem)] gap-0 border border-base-300 rounded-lg overflow-hidden -mx-1">
  <div class="w-56 border-r border-base-300 flex flex-col bg-base-100 shrink-0">
    <div class="p-3 border-b border-base-300 flex items-center justify-between">
      <span class="font-semibold text-sm">{m['developer.edge-functions.panel.title']()}</span>
    ...
    <div class="flex-1 overflow-y-auto">
      ...
      {:else if functions.length === 0}
        <div class="p-4 text-center text-sm text-base-content/40">
          <Code size={28} class="mx-auto mb-2 opacity-30" /> {m['developer.edge-functions.empty.functions']()}
        </div>
      {:else}
    ...
    <div class="flex-1 flex flex-col min-w-0">
      <div class="border-b border-base-300 px-4 py-2 flex items-center justify-between bg-base-100 gap-3">
        <div class="flex items-center gap-3 min-w-0">
          ...
          <label class="label cursor-pointer gap-1 py-0">
            <span class="label-text text-xs">{selected.is_active ? m['common.col.active']() : m['developer.edge-functions.status.inactive']()}</span>
        ...
        <div class="flex gap-2 shrink-0">
          ...
          <button class="btn btn-primary btn-sm gap-1" onclick={saveCode} disabled={saving}>
            {#if saving}<LoaderCircle size={14} class="animate-spin" />{:else if saved}<CheckCircle size={14} />{:else}<Save size={14} />{/if}{m['developer.edge-functions.btn.save']()}
          </button>
        </div>
      </div>
      ...
      <div class="flex flex-1 overflow-hidden">
        ...
        <div class="w-64 border-l border-base-300 flex flex-col bg-base-100 shrink-0">
          <div class="p-3 border-b border-base-300 font-medium text-sm">{m['developer.edge-functions.test.title']()}</div>
          <div class="p-3 flex flex-col flex-1 gap-2">
            <label class="label py-0"><span class="label-text text-xs">{m['developer.edge-functions.ui.request_body_json']()}</span></label>
            ...
            <button class="btn btn-primary btn-sm gap-1 w-full" onclick={invoke} disabled={invoking}>
              {#if invoking}<LoaderCircle size={13} class="animate-spin" />{:else}<Play size={13} />{/if}{m['developer.edge-functions.btn.run']()}
            </button>
            {#if invokeResult}
              <div>
                <div class="flex items-center gap-2 mb-1">
                  <span class="text-xs font-medium">{m['developer.edge-functions.test.response']()}</span>
                ...
                {#if invokeResult.logs?.length}
                  <div class="mt-1"><p class="text-xs font-medium mb-0.5">{m['developer.edge-functions.test.console']()}</p>
  ...
  {:else if !loading}
    <div class="flex-1 flex items-center justify-center text-base-content/40">
      <div class="text-center">
        ...
        <p class="text-sm">{m['developer.edge-functions.empty.selectOrCreate']()}</p>
        <button class="btn btn-primary btn-sm mt-3 gap-1" onclick={() => (showCreateModal = true)}><Plus size={14} /> {m['developer.edge-functions.btn.newFunction']()}</button>
      </div>
    </div>
  {/if}
</div>
...
{#if showCreateModal}
  <div class="modal modal-open">
    <div class="modal-box">
      <h3 class="font-bold text-lg mb-4">{m['developer.edge-functions.ui.new_edge_function']()}</h3>
      <div class="space-y-3">
        <div class="form-control">
          <label class="label py-0"><span class="label-text text-xs">{m['developer.edge-functions.ui.name_url_safe']()}</span></label>
          <input class="input input-sm font-mono" bind:value={form.name} placeholder={m['developer.edge-functions.ui.my_function']()} />
          <label class="label py-0"><span class="label-text-alt text-xs">{m['developer.edge-functions.form.mountedAt']()}{form.name || '…'}</span></label>
        </div>
        <div class="form-control"><label class="label py-0"><span class="label-text text-xs">{m['developer.edge-functions.ui.display_name']()}</span></label><input class="input input-sm" bind:value={form.display_name} placeholder={m['developer.edge-functions.form.displayNamePlaceholder']()} /></div>
        <div class="form-control"><label class="label py-0"><span class="label-text text-xs">{m['developer.edge-functions.ui.http_method']()}</span></label>
      ...
      <div class="modal-action">
        <button class="btn btn-ghost btn-sm" onclick={() => (showCreateModal = false)}>{m['common.cancel']()}</button>
        <button class="btn btn-primary btn-sm" onclick={createFunction} disabled={creating || !form.name || !form.display_name}>
          {#if creating}<LoaderCircle size={13} class="animate-spin" />{/if}{m['developer.edge-functions.btn.create']()}
        </button>
      </div>
    </div>
