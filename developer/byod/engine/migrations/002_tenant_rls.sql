-- 002_tenant_rls.sql
--
-- Multi-tenant isolation for developer/byod. Same pattern as
...
ALTER TABLE zvd_byod_scan_history ADD COLUMN IF NOT EXISTS tenant_id UUID;
ALTER TABLE zvd_byod_scan_profiles ADD COLUMN IF NOT EXISTS tenant_id UUID;

ALTER TABLE zvd_byod_scan_history ALTER COLUMN tenant_id SET DEFAULT COALESCE(NULLIF(current_setting('zveltio.current_tenant', true), '')::uuid, '00000000-0000-0000-0000-000000000001'::uuid);
ALTER TABLE zvd_byod_scan_profiles ALTER COLUMN tenant_id SET DEFAULT COALESCE(NULLIF(current_setting('zveltio.current_tenant', true), '')::uuid, '00000000-0000-0000-0000-000000000001'::uuid);

CREATE INDEX IF NOT EXISTS idx_zvd_byod_scan_history_tenant ON zvd_byod_scan_history (tenant_id);
CREATE INDEX IF NOT EXISTS idx_zvd_byod_scan_profiles_tenant ON zvd_byod_scan_profiles (tenant_id);

ALTER TABLE zvd_byod_scan_history ENABLE ROW LEVEL SECURITY;
ALTER TABLE zvd_byod_scan_history FORCE  ROW LEVEL SECURITY;
ALTER TABLE zvd_byod_scan_profiles ENABLE ROW LEVEL SECURITY;
ALTER TABLE zvd_byod_scan_profiles FORCE  ROW LEVEL SECURITY;

DO $$
DECLARE
  tbl TEXT;
  tables TEXT[] := ARRAY[
    'zvd_byod_scan_history',
    'zvd_byod_scan_profiles'
  ];
BEGIN
...
BEGIN
  ...
  LOOP
    ...
    IF null_count > 0 THEN
      RAISE WARNING
        '[developer/byod tenant_id] % rows in % have tenant_id IS NULL but the engine has provisioned tenants. '
