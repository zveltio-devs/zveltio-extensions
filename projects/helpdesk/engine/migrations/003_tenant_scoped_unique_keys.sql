...
-- zvd_canned_responses: shortcut is unique per company, not per instance.
UPDATE zvd_canned_responses SET tenant_id = '00000000-0000-0000-0000-000000000001'::uuid WHERE tenant_id IS NULL;
ALTER TABLE zvd_canned_responses DROP CONSTRAINT IF EXISTS zvd_canned_responses_shortcut_key;
ALTER TABLE zvd_canned_responses ADD CONSTRAINT zvd_canned_responses_shortcut_key UNIQUE (tenant_id, shortcut);

-- zvd_tickets: number is unique per company, not per instance.
UPDATE zvd_tickets SET tenant_id = '00000000-0000-0000-0000-000000000001'::uuid WHERE tenant_id IS NULL;
ALTER TABLE zvd_tickets DROP CONSTRAINT IF EXISTS zvd_tickets_number_key;
ALTER TABLE zvd_tickets ADD CONSTRAINT zvd_tickets_number_key UNIQUE (tenant_id, number);
