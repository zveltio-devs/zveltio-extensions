-- 002_tenant_rls.sql
--
-- Multi-tenant isolation for sms. Same pattern as
...
ALTER TABLE zv_sms_messages ADD COLUMN IF NOT EXISTS tenant_id UUID;
ALTER TABLE zv_sms_templates ADD COLUMN IF NOT EXISTS tenant_id UUID;

ALTER TABLE zv_sms_messages ALTER COLUMN tenant_id SET DEFAULT COALESCE(NULLIF(current_setting('zveltio.current_tenant', true), '')::uuid, '00000000-0000-0000-0000-000000000001'::uuid);
ALTER TABLE zv_sms_templates ALTER COLUMN tenant_id SET DEFAULT COALESCE(NULLIF(current_setting('zveltio.current_tenant', true), '')::uuid, '00000000-0000-0000-0000-000000000001'::uuid);

CREATE INDEX IF NOT EXISTS idx_zv_sms_messages_tenant ON zv_sms_messages (tenant_id);
CREATE INDEX IF NOT EXISTS idx_zv_sms_templates_tenant ON zv_sms_templates (tenant_id);

ALTER TABLE zv_sms_messages ENABLE ROW LEVEL SECURITY;
ALTER TABLE zv_sms_messages FORCE  ROW LEVEL SECURITY;
ALTER TABLE zv_sms_templates ENABLE ROW LEVEL SECURITY;
ALTER TABLE zv_sms_templates FORCE  ROW LEVEL SECURITY;
...
BEGIN
  ...
  LOOP
    ...
    IF null_count > 0 THEN
      RAISE WARNING
        '[sms tenant_id] % rows in % have tenant_id IS NULL but the engine has provisioned tenants. '
