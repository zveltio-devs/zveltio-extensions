-- 002_tenant_rls.sql
--
-- Multi-tenant isolation for search. Same pattern as
...
ALTER TABLE zv_search_indexes ADD COLUMN IF NOT EXISTS tenant_id UUID;

ALTER TABLE zv_search_indexes ALTER COLUMN tenant_id SET DEFAULT COALESCE(NULLIF(current_setting('zveltio.current_tenant', true), '')::uuid, '00000000-0000-0000-0000-000000000001'::uuid);

CREATE INDEX IF NOT EXISTS idx_zv_search_indexes_tenant ON zv_search_indexes (tenant_id);

ALTER TABLE zv_search_indexes ENABLE ROW LEVEL SECURITY;
ALTER TABLE zv_search_indexes FORCE  ROW LEVEL SECURITY;
...
BEGIN
  ...
  LOOP
    ...
    IF null_count > 0 THEN
      RAISE WARNING
        '[search tenant_id] % rows in % have tenant_id IS NULL but the engine has provisioned tenants. '
