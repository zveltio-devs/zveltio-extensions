...
<div class="space-y-4">
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-xl font-semibold flex items-center gap-2"><Repeat size={20} /> Subscriptions</h1>
      <p class="text-sm text-base-content/50">Manage subscribers, plans, and dunning</p>
    </div>
    {#if tab === 'plans'}
      <button class="btn btn-primary btn-sm gap-1" onclick={() => (showPlanForm = true)}><Plus size={14} /> New plan</button>
    {/if}
  </div>

  <div class="tabs tabs-boxed bg-base-200 w-fit">
    <button class="tab {tab === 'subscribers' ? 'tab-active' : ''}" onclick={() => (tab = 'subscribers')}>Subscribers</button>
    <button class="tab {tab === 'plans' ? 'tab-active' : ''}" onclick={() => (tab = 'plans')}>Plans</button>
    <button class="tab {tab === 'dunning' ? 'tab-active' : ''}" onclick={() => (tab = 'dunning')}>
      <AlertCircle size={13} class="mr-1.5" /> Dunning
    </button>
  </div>
  ...
  {:else if tab === 'subscribers'}
    <div class="overflow-x-auto">
      <table class="table table-sm">
        ...
        <tbody>
          {#if subscribers.length === 0}<tr><td colspan="6" class="text-center py-6 text-base-content/50 text-sm">No subscribers.</td></tr>
  ...
  {:else if tab === 'plans'}
    <div class="overflow-x-auto">
      <table class="table table-sm">
        ...
        <tbody>
          {#if plans.length === 0}<tr><td colspan="5" class="text-center py-6 text-base-content/50 text-sm">No plans yet.</td></tr>
  ...
  {:else}
    <div class="overflow-x-auto">
      <table class="table table-sm">
        ...
        <tbody>
          {#if dunning.length === 0}<tr><td colspan="5" class="text-center py-6 text-base-content/50 text-sm">No failed payments.</td></tr>
  ...
  <div class="modal modal-open">
    <div class="modal-box max-w-md">
      <div class="flex items-center justify-between mb-4">
        <h3 class="font-semibold">New plan</h3>
        <button class="btn btn-ghost btn-xs" onclick={() => (showPlanForm = false)}><X size={14} /></button>
      </div>
      <div class="space-y-3">
        <div class="form-control"><label class="label py-0"><span class="label-text text-xs">Code</span></label><input class="input input-sm font-mono" bind:value={planForm.code} /></div>
        <div class="form-control"><label class="label py-0"><span class="label-text text-xs">Name</span></label><input class="input input-sm" bind:value={planForm.name} /></div>
        <div class="grid grid-cols-2 gap-3">
          <div class="form-control"><label class="label py-0"><span class="label-text text-xs">Price</span></label><input type="number" step="0.01" class="input input-sm" bind:value={planForm.price} /></div>
          <div class="form-control"><label class="label py-0"><span class="label-text text-xs">Currency</span></label><input class="input input-sm" bind:value={planForm.currency} maxlength={3} /></div>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div class="form-control"><label class="label py-0"><span class="label-text text-xs">Interval</span></label>
          ...
          <div class="form-control"><label class="label py-0"><span class="label-text text-xs">Trial days</span></label><input type="number" class="input input-sm" bind:value={planForm.trial_days} /></div>
        </div>
      </div>
      <div class="modal-action">
        <button class="btn btn-ghost btn-sm" onclick={() => (showPlanForm = false)}>Cancel</button>
