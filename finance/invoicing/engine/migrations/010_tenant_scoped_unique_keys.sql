...
-- zvd_credit_notes: number is unique per company, not per instance.
UPDATE zvd_credit_notes SET tenant_id = '00000000-0000-0000-0000-000000000001'::uuid WHERE tenant_id IS NULL;
ALTER TABLE zvd_credit_notes DROP CONSTRAINT IF EXISTS zvd_credit_notes_number_key;
ALTER TABLE zvd_credit_notes ADD CONSTRAINT zvd_credit_notes_number_key UNIQUE (tenant_id, number);

-- zvd_invoices: number is unique per company, not per instance.
UPDATE zvd_invoices SET tenant_id = '00000000-0000-0000-0000-000000000001'::uuid WHERE tenant_id IS NULL;
ALTER TABLE zvd_invoices DROP CONSTRAINT IF EXISTS zvd_invoices_number_key;
ALTER TABLE zvd_invoices ADD CONSTRAINT zvd_invoices_number_key UNIQUE (tenant_id, number);
