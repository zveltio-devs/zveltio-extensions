...
-- zv_page_block_types: name is unique per company, not per instance.
...
-- zv_page_redirects: from_path is unique per company, not per instance.
...
-- zv_pages: slug is unique per company, not per instance.
UPDATE zv_pages SET tenant_id = '00000000-0000-0000-0000-000000000001'::uuid WHERE tenant_id IS NULL;
ALTER TABLE zv_pages DROP CONSTRAINT IF EXISTS zv_pages_slug_key;
ALTER TABLE zv_pages ADD CONSTRAINT zv_pages_slug_key UNIQUE (tenant_id, slug);
