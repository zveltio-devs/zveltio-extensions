...
ALTER TABLE IF EXISTS zv_page_revisions ALTER COLUMN created_by TYPE TEXT;

-- The host creates `zv_pages`; THIS extension adds `updated_by` to it in its own
-- 001_initial.sql, so the conversion belongs here and not in an engine migration
-- — where it would run against a database on which the column does not exist.
ALTER TABLE IF EXISTS zv_pages ALTER COLUMN updated_by TYPE TEXT;
