...
<div class="w-52 shrink-0 bg-base-100 border-r border-base-300 overflow-y-auto flex flex-col">
  <div class="px-3 py-2.5 border-b border-base-300">
    <span class="text-[10px] font-bold text-base-content/65 uppercase tracking-widest">{m['content.pages.b.blocksWord']()}</span>
  </div>

  {#each cats as cat}
    {@const items = LIBRARY.filter(b => b.category === cat.key)}
    <div class="px-2 pt-3 pb-1">
      <p class="text-[9px] font-bold text-base-content/55 uppercase tracking-widest px-1 mb-1">{cat.label()}</p>
      {#each items as item}
        <button
        ...
        >
          ...
          <div class="min-w-0">
            ...
            <p class="text-[10px] text-base-content/65 leading-tight truncate">{item.description}</p>
          </div>
        </button>
      {/each}
    </div>
