 ...
 * element of `items`.
 */
const HTML_KEYS = new Set(['content', 'html', 'code', 'items']);
...
export function bindBlock(block: Any, row: Record<string, Any>, rowKey: string | number): Any {
  ...
  for (const [key, value] of Object.entries(block.content ?? {})) {
    content[key] = bindValue(value, row, rowKey, HTML_KEYS.has(key));
  }
...
function bindValue(
  ...
  if (value && typeof value === 'object') {
    ...
    for (const [k, v] of Object.entries(value)) {
      out[k] = bindValue(v, row, rowKey, intoHtml || HTML_KEYS.has(k));
    }
    return out;
  }
  return value;
