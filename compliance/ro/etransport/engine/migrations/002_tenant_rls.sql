-- 002_tenant_rls.sql
--
-- Multi-tenant isolation for compliance/ro/etransport. Same pattern as
...
ALTER TABLE zv_etransport_declarations ADD COLUMN IF NOT EXISTS tenant_id UUID;

ALTER TABLE zv_etransport_declarations ALTER COLUMN tenant_id SET DEFAULT COALESCE(NULLIF(current_setting('zveltio.current_tenant', true), '')::uuid, '00000000-0000-0000-0000-000000000001'::uuid);

CREATE INDEX IF NOT EXISTS idx_zv_etransport_declarations_tenant ON zv_etransport_declarations (tenant_id);

ALTER TABLE zv_etransport_declarations ENABLE ROW LEVEL SECURITY;
ALTER TABLE zv_etransport_declarations FORCE  ROW LEVEL SECURITY;

DO $$
DECLARE
  tbl TEXT;
  tables TEXT[] := ARRAY[
    'zv_etransport_declarations'
  ];
BEGIN
...
BEGIN
  ...
  LOOP
    ...
    IF null_count > 0 THEN
      RAISE WARNING
        '[compliance/ro/etransport tenant_id] % rows in % have tenant_id IS NULL but the engine has provisioned tenants. '
