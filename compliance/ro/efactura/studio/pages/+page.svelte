...
<div class="space-y-4">
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-xl font-semibold flex items-center gap-2"><FileText size={20} /> e-Factura RO</h1>
      <p class="text-sm text-base-content/50">Genereaza si trimite facturi UBL XML la ANAF</p>
    </div>
    <button class="btn btn-primary btn-sm gap-1" onclick={() => (showCreateModal = true)}><Plus size={14} /> New Invoice</button>
  </div>
  ...
  {:else}
    <div class="overflow-x-auto">
      <table class="table table-sm">
        <thead><tr><th>Number</th><th>Date</th><th>Buyer</th><th>Total</th><th>Status</th><th>ANAF Index</th><th></th></tr></thead>
        <tbody>
            ...
            <tr class="hover">
              ...
              <td>
                <div class="flex gap-1">
                  {#if inv.status === 'draft'}
                    <button class="btn btn-ghost btn-xs" onclick={() => generateXML(inv.id)}>XML</button>
                  {/if}
    ...
    <div class="modal-box w-11/12 max-w-4xl">
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div class="form-control">
          <label class="label" for="inv-number"><span class="label-text">Invoice number</span></label>
          <input id="inv-number" type="text" bind:value={form.invoice_number} placeholder="FAC-2026-001" class="input input-sm" />
        </div>
        <div class="form-control">
          <label class="label" for="inv-currency"><span class="label-text">Currency</span></label>
        ...
        <div class="form-control">
          <label class="label" for="inv-date"><span class="label-text">Invoice date</span></label>
          <input id="inv-date" type="date" bind:value={form.invoice_date} class="input input-sm" />
        </div>
        <div class="form-control">
          <label class="label" for="inv-due"><span class="label-text">Due date</span></label>
      ...
      <div class="mb-4">
        ...
        <div class="overflow-x-auto">
          <table class="table table-xs">
            <thead><tr><th>Description</th><th>Qty</th><th>Unit</th><th>Price</th><th>VAT%</th><th>VAT</th><th>Total</th><th></th></tr></thead>
            <tbody>
      ...
      <div class="modal-action">
        ...
        <button class="btn btn-primary" onclick={createInvoice} disabled={creating || !form.invoice_number || !form.seller_name || !form.buyer_name}>
          {#if creating}<span class="loading loading-spinner loading-sm"></span>{/if} Create Invoice
        </button>
      </div>
    </div>
