-- 002_tenant_rls.sql
--
-- Multi-tenant isolation for forms. Same pattern as
...
ALTER TABLE zv_form_submissions ADD COLUMN IF NOT EXISTS tenant_id UUID;
ALTER TABLE zv_forms ADD COLUMN IF NOT EXISTS tenant_id UUID;

ALTER TABLE zv_form_submissions ALTER COLUMN tenant_id SET DEFAULT COALESCE(NULLIF(current_setting('zveltio.current_tenant', true), '')::uuid, '00000000-0000-0000-0000-000000000001'::uuid);
ALTER TABLE zv_forms ALTER COLUMN tenant_id SET DEFAULT COALESCE(NULLIF(current_setting('zveltio.current_tenant', true), '')::uuid, '00000000-0000-0000-0000-000000000001'::uuid);

CREATE INDEX IF NOT EXISTS idx_zv_form_submissions_tenant ON zv_form_submissions (tenant_id);
CREATE INDEX IF NOT EXISTS idx_zv_forms_tenant ON zv_forms (tenant_id);

ALTER TABLE zv_form_submissions ENABLE ROW LEVEL SECURITY;
ALTER TABLE zv_form_submissions FORCE  ROW LEVEL SECURITY;
ALTER TABLE zv_forms ENABLE ROW LEVEL SECURITY;
ALTER TABLE zv_forms FORCE  ROW LEVEL SECURITY;
...
BEGIN
  ...
  LOOP
    ...
    IF null_count > 0 THEN
      RAISE WARNING
        '[forms tenant_id] % rows in % have tenant_id IS NULL but the engine has provisioned tenants. '
